...
class Transaction(MyModel, table=True):
    ...
    async def account(self, info, parameters):
        return await info.context['dl_account'].load([self.bank_id, self.account_id], parameters)


async def dl_transaction_account(keys: list[tuple]) -> list[Transaction.schema]:
